					...
					<tbody>
 						<form method="post" action="{{ route('update.product.multiimage') }}" enctype="multipart/form-data" >
							...
							<tr>
								...
								<td> <img src="{{ asset($img->photo_name) }}" style="width:70; height: 40px;" id="MultiShowImage{{ $img->id }}"> </td>
								<td> <input type="file" class="form-group" name="multi_img[{{ $img->id }}]" id="MultiImage{{ $img->id }}"> </td>
								<td> 
...
<script> 
 	$(document).ready(function(){
        $('#MultiImage{{ $img->id }}').change(function(e){
            var reader = new FileReader();
            reader.onload = function(e){
                $('#MultiShowImage{{ $img->id }}').attr('src',e.target.result);
            }
            reader.readAsDataURL(e.target.files['0']);
        })
    });
